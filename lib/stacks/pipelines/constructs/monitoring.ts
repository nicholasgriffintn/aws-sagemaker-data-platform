...
export interface ExperimentEndpointMonitoringProps {
...
export interface ExperimentEndpointMonitoringResources {
	alertsTopic: Topic;
...
export class ExperimentEndpointMonitoring extends Construct {
	...
	constructor(scope: Construct, id: string, props: ExperimentEndpointMonitoringProps) {
		super(scope, id);

		this.alertsTopic = new Topic(this, "ExperimentAlertsTopic", {
			topicName: `${props.componentName}-${props.environmentName}-experiment-alerts`,
			displayName: props.displayName ?? "Experiment Pipeline Alerts",
		});
		...
		this.scalingPolicy = new TargetTrackingScalingPolicy(this, "EndpointScalingPolicy", {
			scalingTarget: this.scalableTarget,
			targetValue: props.invocationTargetValue ?? 100,
			predefinedMetric: PredefinedMetric.SAGEMAKER_VARIANT_INVOCATIONS_PER_INSTANCE,
