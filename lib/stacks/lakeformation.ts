...
export class LakeFormationStack extends Stack {
  constructor(scope: Construct, id: string, props: LakeFormationStackProps) {
    ...
    const admins = props.dataLakeAdmins.length > 0 ? props.dataLakeAdmins : [props.pipelineRole];
    ...
    const rawDatabaseName = `${props.componentName}_${props.environmentName}_raw`;
    const processedDatabaseName = `${props.componentName}_${props.environmentName}_processed`;
    ...
    const datasetConfigs = [
      {
        ...
        databaseName: rawDatabaseName,
      },
      {
        id: 'Processed',
        tagValue: 'processed',
        dataLocationArn: props.processedDataBucket.bucketArn,
        databaseName: processedDatabaseName,
      },
    ];

    const datasetTagValues = Array.from(new Set(datasetConfigs.map((dataset) => dataset.tagValue)));
    ...
    const databaseTagAssociations: Record<string, lakeformation.CfnTagAssociation> = {};

    for (const dataset of datasetConfigs) {
      if (!dataset.databaseName) {
        continue;
      }
      const association = new lakeformation.CfnTagAssociation(this, `${dataset.id}DatabaseDatasetTag`, {
        resource: {
          database: {
            catalogId,
    ...
    for (const dataset of datasetConfigs) {
      const dataLocation = new lakeformation.CfnResource(this, `${dataset.id}DataLocation`, {
    ...
    for (const principal of principals) {
      for (const dataset of datasetConfigs) {
        const dataLocationPermission = new lakeformation.CfnPrincipalPermissions(this, `${principal.id}${dataset.id}DataLocationAccess`, {
          principal: {
        ...
        const databasePermission = new lakeformation.CfnPrincipalPermissions(this, `${principal.id}${dataset.id}DatabaseTagPolicy`, {
          principal: {
            dataLakePrincipalIdentifier: principal.role.roleArn,
          },
        ...
        const tablePermission = new lakeformation.CfnPrincipalPermissions(this, `${principal.id}${dataset.id}TableTagPolicy`, {
          principal: {
            dataLakePrincipalIdentifier: principal.role.roleArn,
          },
